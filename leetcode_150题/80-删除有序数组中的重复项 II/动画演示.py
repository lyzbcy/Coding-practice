"""
LeetCode 80. 删除有序数组中的重复项 II - 动画演示
...
class RemoveDuplicatesAnimation:
    def __init__(self, nums):
        ...
        self.fig.canvas.manager.set_window_title('删除有序数组中的重复项 II - 动画演示（手动控制）')
        self.ax.axis('off')
    ...
    def _simulate_algorithm(self):
        ...
        self.steps.append({
            ...
            'action': f'初始化：slow=0 指向首个元素 {self.nums[slow]}，fast=1 开始扫描',
            'code_highlight': [0, 1, 2, 3],
            'explanations': [
                'nums[0] 作为第一个元素已经就位（最多允许两个相同元素）。',
                'fast 将从索引 1 开始检查是否应该保留。'
            ]
        })

        for fast in range(1, self.length):
            current = self.nums[fast]
            # 判断条件：slow < 1 或 nums[fast] != nums[slow-1]
            should_write = slow < 1 or self.nums[fast] != self.nums[slow - 1]
            
            if not should_write:
                # 已经有两个相同元素了，跳过
                action = (f'fast={fast}，nums[{fast}]={current} 与 nums[{slow-1}]={self.nums[slow-1]} 相同，'
                          f'且 nums[{slow}]={self.nums[slow]} 也相同，已出现两次，跳过')
                code_lines = [3, 4]
                explanations = [
                    f'当前值 {current} 已经在位置 {slow-1} 和 {slow} 出现了两次。',
                    '根据题目要求，每个元素最多保留两次，所以跳过这个值。'
                ]
                write_index = None
            else:
                # 可以写入
                ...
                write_index = slow
                if slow < 2:
                    action = (f'fast={fast}，nums[{fast}]={current}，slow < 2，前两个元素可直接保留，'
                              f'写入 slow={slow} 的位置')
                    explanations = [
                        f'当前 slow={slow} < 2，前两个元素肯定可以保留。',
                        f'覆盖写入 nums[{slow}]，保证前 {slow+1} 个位置符合要求。'
                    ]
                else:
                    action = (f'fast={fast}，nums[{fast}]={current} != nums[{slow-1}]={self.nums[slow-1]}，'
                              f'可以保留，写入 slow={slow} 的位置')
                    explanations = [
                        f'nums[{fast}]={current} 与 nums[{slow-1}]={self.nums[slow-1]} 不同，可以写入。',
                        f'或者该值只出现了一次，也可以写入。'
                    ]
        ...
        self.steps.append({
            ...
            'code_highlight': [9, 10],
            'explanations': [
                f'数组前 {slow + 1} 个位置已全部填充符合要求的元素（每个元素最多出现两次）。',
                '后续元素可以忽略，不影响判题。'
            ]
        })
    ...
    def _draw_step(self, step_index=None):
        ...
        self.ax.text(-0.5, 4.8, f'当前有效元素数量（slow+1）= {step_data["slow"] + 1 if self.length else 0}', fontsize=12,
...
def main():
    print("=" * 60)
    print("LeetCode 80. 删除有序数组中的重复项 II - 动画演示")
    print("=" * 60)
    print("\n选择示例：")
